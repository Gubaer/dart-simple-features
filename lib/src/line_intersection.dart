...
class LineSegment {
   ...
   *
   */
  LineSegment(DirectPosition2D start, DirectPosition2D end) {
  ...
  double get slope => (_end.y - _start.y).toDouble() /
                      (_end.x - _start.x).toDouble();
...
class _Event implements Comparable<_Event> {
  ...
  add(LineSegment segment) {
    if (_segments == null) _segments = new List<LineSegment>();
...
class _EventQueue {
  ...
  /// events position
  var _events = new SplayTreeMap<_Event, _Event>();
  ...
  addLineSegmentEvents(LineSegment segment) {
    ...
    _events[new _Event(segment.start)].add(segment);
  }
   ...
   */
  _Event unshift() {
    if (isEmpty) throw new StateError();
...
class _SweepLineCompareFunction {
  ...
  int call(LineSegment value, LineSegment other) {
    ...
    // equivalence class.
    return sign * value.counterclockwiseOrientation.compareTo(other.counterclockwiseOrientation);
  }
}
...
class LineIntersection {
  ...
  List<LineSegment> get intersecting => new UnmodifiableListView(_intersecting);

  LineIntersection._(this.pos, Iterable<LineSegment> intersecting) {
    _require(intersecting != null);
    this._intersecting = new List.from(intersecting, growable: false);
  }

  toString() => "{intersection: pos=$pos, segments=${_intersecting.join(",")}";
}

final _logger = LoggerFactory.getLogger("LineIntersection");
 ...
 * [segments].
 */
Iterable<LineIntersection> computeLineIntersections(Iterable<LineSegment> segments) {
  ...
  handleEvent(_Event event) {
    ...
    if (U.length + L.length + C.length > 1) {
      ...
      var intersecting = flatten([U, L, C]).toSet();
      intersections.add(new LineIntersection._(event.pos, intersecting));
    }
...
//    U = flatten(U).toList(growable: false);
    ...
    compare = (LineSegment other) => _orientation(other.start, other.end, event.pos);
